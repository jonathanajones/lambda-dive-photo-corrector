...
def color_correct_image(image_string):
    # image_bytes = base64.b64decode(image_string)
    image_bytes = io.BytesIO(base64.b64decode(image_string)).getvalue()
    mat = cv2.imdecode(np.frombuffer(image_bytes, np.uint8), cv2.IMREAD_UNCHANGED)
    ...
    _is_success, corrected_bytes = cv2.imencode(".jpg", corrected_mat)
    io_buf = io.BytesIO(corrected_bytes)
    image_str = base64.b64encode(io_buf.getvalue()).decode("utf-8")
    return image_str


def lambda_handler(event: Dict, _context) -> dict:
    img_str = color_correct_image(event["image"])

    return {
        "statusCode": 200,
        "data": {"imageType": "image/jpg", "imageData": img_str},
    }


if __name__ == "__main__":
    # very simple harness to test locally
    with open("image.jpg", "rb") as im:
        file = im.read()
        image = base64.b64encode(file).decode()
        event: Dict[str, str] = {"image": image}
        response: Dict[str, Any] = lambda_handler(event, None)
        with open("out.jpg", "wb") as file:
            image_buffer: bytes = base64.b64decode(response["data"]["imageData"])
            file.write(image_buffer)
        print(
            {
                "statusCode": 200,
                "data": {"imageType": "image/jpg", "imageData": "..."},
            }
        )
